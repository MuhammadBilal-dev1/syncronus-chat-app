...
const MultipleSelector = React.forwardRef(
  (
    ...
    const EmptyItem = React.useCallback(() => {
      ...
      // Agar user ne kuch type nahi kiya, toh "No results" dikhane ki zaroorat nahi
      if (inputValue.length === 0) return null;

      if (onSearch && !creatable && Object.keys(options).length === 0) {
        return (
          <CommandItem value="-" disabled className="text-center text-sm py-2">
            {emptyIndicator}
          </CommandItem>
        );
      }

      // Sirf tab dikhayein jab waqai search result 0 ho
      return (
        <CommandEmpty className="py-6 text-center text-sm">
          {emptyIndicator}
        </CommandEmpty>
      );
    }, [creatable, emptyIndicator, onSearch, options, inputValue]);
    ...
    const hasSelectableOptions = Object.values(selectables).some(
    ...
    return (
      ...
      >
        <div
          className={cn(
            "min-h-10 rounded-md border-0 text-sm ring-offset-background focus-within:ring-0",
            {
        ...
        <div className="relative">
          {open && hasSelectableOptions && (
            <CommandList
              className="absolute top-2 z-10 w-full rounded-md border border-slate-800 bg-[#1c1d25] text-white shadow-xl outline-none animate-in"
              onMouseLeave={() => {
...
function removePickedOption(groupOption, picked) {
  ...
  // Fix: Check karo ke picked waqai ek array hai
...
function isOptionsExist(groupOption, targetOption) {
  // Check if targetOption is valid array
  if (!Array.isArray(targetOption)) return false;
